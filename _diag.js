...
(async () => {
  ...
  if (logs.data && logs.data.deploymentLogs) {
    // Show all non-QR-art logs
    const useful = logs.data.deploymentLogs.filter(l => !l.message.match(/^[█▄▀ ]+$/) && l.message.trim());
    useful.forEach(l => console.log(`[${l.severity || 'info'}] ${l.message}`));
  }
})();
